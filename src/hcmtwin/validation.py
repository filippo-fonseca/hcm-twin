...
def _bool_row(gate: str, quantity: str, passed: bool, note: str) -> GateRow:
    return GateRow(
        gate=gate, quantity=quantity, value=float(passed), units="boolean",
        target="true", passed=bool(passed), note=note,
    )
...
def _sweep(modifier: Callable[[float], object], factors: tuple[float, ...]):  # type: ignore[no-untyped-def]
    return [
        _run(HEALTHY_GEOMETRY, HEALTHY_MATERIAL, modifier(f))[0] for f in factors
    ]
...
def validation_rows() -> pd.DataFrame:
    ...
    healthy, healthy_result = _run(
        HEALTHY_GEOMETRY, HEALTHY_MATERIAL, RESTING_LOADING, trace=True
    )
    ...
    rows.append(
        _bool_row(
            "healthy baseline", "not obstructive",
    ...
    rows.append(
        _bool_row(
            "healthy baseline", "steady state reached", healthy_result.converged,
    ...
    rows.append(
        _bool_row(
            "Frank-Starling", "EDV rises with preload",
    ...
    rows.append(
        _bool_row(
            "Frank-Starling", "SV rises with preload",
    ...
    # --- Gate 3: afterload ------------------------------------------------------------
    afterload = _sweep(lambda f: RESTING_LOADING.scaled(resistance_factor=f), (0.85, 1.0, 1.15, 1.30))
    rows.append(
        _bool_row(
            "afterload", "SV falls with afterload",
    ...
    rows.append(
        _bool_row(
            "afterload", "ESV rises with afterload",
    ...
    rows.append(_bool_row("loop shape", "loop closes", closes,
                          f"gap {abs(float(volume[0] - volume[-1])):.3f} mL over {span:.1f} mL"))
    rows.append(_bool_row("loop shape", "counter-clockwise", signed_area > 0,
                          f"stroke work {signed_area:.0f} mmHg*mL"))
    rows.append(_bool_row("loop shape", "isovolumic phases present",
    ...
    rows.append(_bool_row("diastolic", "passive relation monotone",
                          bool(np.all(np.diff(passive) > 0)), "at fixed volume"))
    rows.append(_bool_row("diastolic", "passive relation convex",
                          bool(np.all(np.diff(passive, 2) > -1e-9)), "at fixed volume"))
    edps = [
        _run(HEALTHY_GEOMETRY, replace(HEALTHY_MATERIAL, a_pas_kpa=a), RESTING_LOADING)[0]
        .end_diastolic_pressure_mmhg
        for a in (0.6, 0.9, 1.5, 2.5, 4.0)
    ]
    rows.append(_bool_row("diastolic", "stiffer tissue raises filling pressure",
                          _monotone(edps, True),
    ...
    checks: list[tuple[str, bool, str]] = [
        ("supranormal ejection fraction", hcm.ejection_fraction >= 0.70,
         f"{hcm.ejection_fraction:.3f} vs healthy {healthy.ejection_fraction:.3f}"),
        ("reduced stroke volume", hcm.stroke_volume_ml < healthy.stroke_volume_ml,
        ...
        ("elevated E/e' surrogate", hcm.e_over_e_prime > healthy.e_over_e_prime,
        ...
        ("hypertrophic on imaging", hcm.wall_thickness_cm >= d.TRIAL_MIN_WALL_THICKNESS_CM,
         f"{hcm.wall_thickness_cm:.2f} cm"),
        ("disease needs material, not only geometry",
         bool(benign.end_diastolic_pressure_mmhg < hcm.end_diastolic_pressure_mmhg
    ...
    rows.append(_bool_row(
        "drug direction", "dose lowers ejection fraction",
        _monotone([o.ejection_fraction for o in ladder], False),
        "EF " + ", ".join(f"{o.ejection_fraction:.3f}" for o in ladder),
    ))
    rows.append(_bool_row(
        "drug direction", "dose lowers outflow gradient",
