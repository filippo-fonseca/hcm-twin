...
The model reproduces \nGatesPassed{} of \nGates{} prespecified validation gates and makes
two quantitative predictions that were not fitted: an over-response rate of
\overResponderRate\% in a virtual cohort built with the pivotal trial's own enrolment
criteria, against a published 5.2\%, and a \efChangeMid{}-point fall in ejection fraction at
the mid dose, against a placebo-corrected 4.8 points reported for aficamten.
...
The specification this work followed prescribed $\sigma_a = T_\text{ref} A$. Three additions
were needed, each because the model visibly failed without it, and each documented with its
failure:

\begin{itemize}
  \item \textbf{Active force--length (filament overlap).} Cavity pressure is
        $\sigma_f/(1+3V/V_w)$, so at fixed fiber stress a \emph{smaller} cavity makes
        \emph{more} pressure. With no length dependence the chamber's pressure-generating
        capacity rises monotonically as it empties (141\,mmHg at 100\,mL, 224 at 42,
        314 at 13): a \emph{negative} end-systolic elastance. Ejection then never meets a
        force balance.
  \item \textbf{Cross-bridge distortion,} giving a force--velocity relation. Without it
        shortening speed is unbounded and the first coupled run ejected past zero volume.
  \item \textbf{Force-dependent recruitment from the parked state,} the published mechanism
        for length-dependent activation. Calcium-sensitivity length dependence alone gave a
        stroke volume that was flat then falling as preload rose. It is also what makes
        contractile reserve mechanistically real: a depleted parked pool cannot be recruited
        when load rises.
\end{itemize}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.62\linewidth]{fig_pv_loops.png}
...
\paragraph{Calibration honesty.} The healthy and HCM references were calibrated by grid
search against resting haemodynamic \emph{ranges}, never against a trial outcome. Three
ejection-realism targets (peak outflow velocity, ejection duration, peak aortic flow) were
added after a first calibration passed every haemodynamic gate while ejecting the whole
stroke volume in 154\,ms at twice physiological peak flow. A model can satisfy its checks and
still be wrong.
...
\claim{\overResponderRate\% of them cross the ejection-fraction floor at or below the mid
dose}, against a published 5.2\%. Nothing in the priors was tuned to that number.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\linewidth]{fig_over_responders.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{fig_sensitivity_hidden.png}
...
noise were treated as data and the posterior over the five hidden parameters recovered by
Markov chain Monte Carlo. Sampling ran against a per-patient polynomial emulator fitted on
\nSurrogateDesign{} design points, whose held-out coefficient of determination is at worst
\surrogateMinRsq{} and typically \surrogateMedianRsq. The emulator's error is added in
quadrature to the measurement noise, so imprecision widens the posterior rather than
corrupting it, and the synthetic data always come from the exact model so that emulator
error acts as misspecification rather than cancelling out.
...
baseline observables \emph{plus} the maneuver's, and scored the drop in correlation, the
shrinkage of the credible intervals, and the discriminating signal in clinical units against
that observable's measurement error. The signal is computed by displacing the truth along the
\emph{confounded direction} by the amount a resting study cannot resolve, and asking how far
apart the two indistinguishable patients look under the maneuver.

\input{table_tiebreaker}
\captionof{table}{The tie-breaker table. ``Beats noise?'' compares the discriminating signal
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{fig_tiebreaker.png}
...
The best case is \bestPair{} under \bestManeuver, where the posterior correlation falls from
\bestCorrelationBefore{} to \bestCorrelationAfter{} and the discriminating signal is
\bestSignal\,\bestSignalUnits{} in \bestSignalObservable, or \bestSignalToNoise{} measurement
standard deviations (exceeds noise: \bestExceedsNoise). \nPairsSeparated{} of \nPairsTested{}
examined pairs produce a signal above one measurement error. The maneuver was interpretable
in \bestManeuverUsable\% of the patients it was applied to; the remainder are ventricles too
stiff to fill at the provoked heart rate, which is not a numerical failure but the maneuver
being uninterpretable in exactly the patients it was proposed for.
...
The tissue-level story is more mixed and is reported in Table~\ref{tab:recovery}. Some
material parameters are recoverable from a routine study; the two passive stiffness
parameters trade off against each other, which is unsurprising given that they are two arms
of one exponential; and a maneuver helps in some cases by an amount that a real clinic could
or could not measure, which is exactly the distinction the table is built to draw.
...
\begin{itemize}
  ...
  \item \textbf{Zero-dimensional chamber.} Homogeneous wall loading, so no fiber disarray, no
        regional fibrosis, no asymmetric hypertrophy, no systolic anterior motion as an
        explicit mechanism. HCM hypertrophy is characteristically asymmetric and this model
        has a uniformly thickened wall of the same total volume.
  ...
  \item \textbf{No growth or remodelling.} Wall volume is sampled, not grown. The model
        describes a geometry-plus-material combination today and says nothing about how it
        arose or where it is heading.
  ...
  \item \textbf{No mutation-specific claims.} The mapping from a named variant to a numerical
        parameter is literature-derived and approximate. Every claim here is about regions of
        parameter space, never about named patients.
  \item \textbf{Nothing here is validated against a patient.} The comparisons to trial data
        are comparisons of aggregate behaviour to published aggregates.
\end{itemize}

\section{Reproducibility}

One command reproduces every figure, and every figure ships with the CSV behind it.
\texttt{make all} inside the supplied container runs the test suite (including all
\nGates{} validation gates and a provenance-coverage test that fails if any constant lacks a
sourced row), generates the virtual population from a logged seed (\seed), and produces every
table, figure and this document. Total wall-clock time for the run described here was
\runtimeMinutes{} minutes. Every quantitative statement in this paper is a macro resolved from
a results file at build time; none is typed.

\vspace{4pt}
\noindent\textbf{Data and code.} The research dossier in \texttt{docs/research/} records the
source of every constant, every simplification made relative to it, and every place the
literature failed to supply what the model needed. Those gaps are marked and described rather
than filled with plausible values.
